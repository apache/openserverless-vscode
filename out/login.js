...
var Validators = {
  minChars: (htmlElement, min, fieldName = null) => {
    const message = `Field '${fieldName || htmlElement.id}' should contain minimum ${min} chars.`;
    let isValid = true;
    isValid = htmlElement.value.length >= min;
    return { isValid, message, htmlElement };
  },
  maxChars: (htmlElement, max, fieldName = null) => {
    const message = `Field '${fieldName || htmlElement.id}' should contain maximum ${max} chars.`;
    let isValid = true;
    isValid = htmlElement.value.length <= max;
    return { isValid, message, htmlElement };
  },
  validUrl: (htmlElement, fieldName = null) => {
    const message = `Field '${fieldName || htmlElement.id}' should be a valid URL.`;
  ...
  isNotNullOrUndefined: (htmlElement, fieldName = null) => {
    const message = `Field '${fieldName || htmlElement.id}' should not be null or undefined.`;
    const isValid = htmlElement.value !== null && htmlElement.value !== void 0;
    return { isValid, message, htmlElement };
  },
  isNot: (htmlElement, value, fieldName = null) => {
    const message = `Field '${fieldName || htmlElement.id}' should not be ${value}.`;
    const isValid = htmlElement.value !== value;
    return { isValid, message, htmlElement };
  }
};
function validateNotEmptyField(fieldName, input) {
  const assertions = [];
...
function createErrorMessages(assertions) {
  try {
    ...
    assertions.filter((a) => !a.isValid).forEach((assertion, i) => {
...
var LoginPage = {
  getUsername: () => {
    return document.getElementById("username-input" /* Username */);
  },
  getPassword: () => {
    return document.getElementById("password-input" /* Password */);
  },
  getApiHost: () => {
    return document.getElementById("api-host-input" /* ApiHost */);
  },
  getLoginButton: () => {
